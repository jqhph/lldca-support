...
class Validator
{
    ...
    protected static $typeMsg = [
        ...
        'boolean'     => ':attr must be bool',
        'email'       => ':attr not a valid email address',
        'array'       => ':attr must be a array',
        'accepted'    => ':attr must be yes,on or 1',
        'date'        => ':attr not a valid datetime',
        'file'        => ':attr not a valid file',
        'image'       => ':attr not a valid image',
        'alpha'       => ':attr must be alpha',
        'alphaNum'    => ':attr must be alpha-numeric',
        'alphaDash'   => ':attr must be alpha-numeric, dash, underscore',
        'activeUrl'   => ':attr not a valid domain or ip',
        'chs'         => ':attr must be chinese',
        'chsAlpha'    => ':attr must be chinese or alpha',
        'chsAlphaNum' => ':attr must be chinese,alpha-numeric',
        'chsDash'     => ':attr must be chinese,alpha-numeric,underscore, dash',
        'url'         => ':attr not a valid url',
        'ip'          => ':attr not a valid ip',
        'dateFormat'  => ':attr must be dateFormat of :rule',
        'in'          => ':attr must be in :rule',
        'notIn'       => ':attr be notin :rule',
        'between'     => ':attr must between :1 - :2',
        'notBetween'  => ':attr not between :1 - :2',
    ...
    {
        // 支持多规则验证 required|in:a,b,c|... 或者 ['required','in'=>'a,b,c',...]
        if (is_string($rules)) {
            $rules = explode('|', $rules);
        }
        $i = 0;
            ...
            if (false === $result) {
                ...
                if (isset($msg[$i])) {
                    ...
                    if (is_string($message) && strpos($message, '{%') === 0) {
                        $message = $this->translate(substr($message, 2, -1));
                    }
                } else {
                ...
                if (is_string($result) && false !== strpos($result, ':')) {
                    $result = str_replace([':attr', ':rule'], [$title, (string) $rule], $result);
                }
                return $result;
            }
            $i++;
    ...
    protected function image($file, $rule)
    {
        ...
        $path = array_get($file->toArray(), 'tmp_file');
        ...
        if ($rule) {
            ...
            return in_array($this->getImageType($path), [1, 2, 3, 6]);
        }
    }
    ...
    protected function getRuleMsg($attribute, $title, $type, $rule)
    {
        ...
        } elseif (0 === strpos($type, 'required')) {
            $msg = self::$typeMsg['required'];
        } else {
            $msg = $title . $this->translate('not conform to the rules');
        }
